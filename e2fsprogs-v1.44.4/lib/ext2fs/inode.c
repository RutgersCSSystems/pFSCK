...
int get_group_no(ext2_inode_scan scan){
	return scan->current_group;
}
...
/* Splits used inodes in a filesystem into some fixed set of virtual groups,
 * where each group consists of a coninuout range of inodes to process
 *
 * @param  fs     filesystem structure
 * @param  count  number of virtual groups to create
 *
 * @return  an array of virtual group structures of length "count"
 */
struct vgroup* split_into_vgroups(ext2_filsys fs, int count, struct vgroup **vpointer){
  int total_inodes_to_check = 0; // # of inodes to check
  int inodes_per_vgroup;         // # of inodes that should be assigned
  struct vgroup *vgroups;  	 // List of vgroups to return
  int x;                         // temp counter
  int unused_inodes;		 // temp var to hold unused inode count
  int *inode_counts;		 // list of ints to records inodes per group
  int total_inodes_per_group;	 // total nummber of inodes per group
  int total_group_count;

  //printf("Splitting into Vgroups");

  // Get what we know:
  total_inodes_per_group = EXT2_INODES_PER_GROUP(fs->super);
  total_group_count = fs->group_desc_count;

  // Allocate list structures
  inode_counts = (int*) malloc(sizeof(int) * total_group_count);
  vgroups = (struct vgroup*) malloc(sizeof(struct vgroup) * count);
  //printf("Allocated vgroups at %x", vgroups);
  int group_no;

  // Get total number of inodes and keep track of how many inodes to check
  // per group
  for(group_no = 0; group_no < total_group_count; group_no++){
		unused_inodes = ext2fs_bg_itable_unused(fs, group_no);
		inode_counts[group_no] = total_inodes_per_group - unused_inodes;
    total_inodes_to_check += inode_counts[group_no];
  }

  // Calculate how many inodes per group
  inodes_per_vgroup = total_inodes_to_check / count;
  if(total_inodes_to_check % count > 0){
    inodes_per_vgroup++;
  }

 int curr_group_number = 0;
 int curr_group_offset = 0;
 int inode_quota;

 // Go through disk groups to assign
 for(x = 0; x < count; x++){
   inode_quota = inodes_per_vgroup;

   if(x != 0){
     curr_group_offset++;
     if(curr_group_offset > inode_counts[curr_group_number] - 1){
	     curr_group_number++;
	     curr_group_offset = 0;
     }
   }
   ...
   // Assign starting group and offset
   vgroups[x].start_group = curr_group_number;
   vgroups[x].start_offset = curr_group_offset;

   // Scan through groups to fufill inode quota
   while(curr_group_number < total_group_count){
     inode_quota -= (inode_counts[curr_group_number] - curr_group_offset);
     
     curr_group_offset = inode_counts[curr_group_number];

     if(inode_quota <= 0){
       curr_group_offset = inode_counts[curr_group_number] - 1 - (0 - inode_quota);
       inode_quota = 0;
       break;
     }
   ...
   // Assign ending group and offset number
   vgroups[x].end_group = curr_group_number;
   vgroups[x].end_offset = curr_group_offset;

   // calculate inode numbers
   vgroups[x].start_inode = vgroups[x].start_group * EXT2_INODES_PER_GROUP(fs->super) + 1 + vgroups[x].start_offset;
   vgroups[x].end_inode = vgroups[x].end_group * EXT2_INODES_PER_GROUP(fs->super) + 1 + vgroups[x].end_offset;
   vgroups[x].total_inodes = inodes_per_vgroup - inode_quota; 
 }

 /*
 printf("Printing Group Info");
 for(x = 0; x < total_group_count; x++){
	 printf("Group No %d: %d inodes\n", x, inode_counts[x]);
 }

 printf("Printing Vgroups\n");
 for(x = 0; x < count; x++){
	 printf("start_g: %d, start_offset: %d, end_g: %d, end_offset: %d\n",\
          vgroups[x].start_group, vgroups[x].start_offset, \
					vgroups[x].end_group, vgroups[x].end_offset);
 ...
 */
 ...
 // Return the list of vitual groups
 //printf("Returning vgroups %x", vgroups);
 *vpointer = vgroups;
 return vgroups;

}

int get_start_inode(struct vgroup *v){
	return v->start_inode;
}

int get_end_inode(struct vgroup *v){
	return v->end_inode;
}

int get_start_group(struct vgroup *v){
	return v->start_group;
}

int get_end_group(struct vgroup *v){
	return v->end_group;
}

void get_vgroup(struct vgroup *vgroups, int index, struct vgroup **v){
  //printf("vgroups value: %p getting %d, ptr: %p\n", vgroups, index, &vgroups[index]);
  *v = &vgroups[index];
  //printf("Gonna start at %d\n", vgroups[index].start_group);
  //printf("Gonna start at inode %d\n", vgroups[index].start_inode);
}


void free_vgroups(struct vgroup* vgroups){
	free(vgroups);
	//printf("Freed vgroups");
}
