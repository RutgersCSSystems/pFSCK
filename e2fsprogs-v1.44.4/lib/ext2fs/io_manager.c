...
errcode_t io_channel_check_cache_blk(io_channel channel, unsigned long long block)
{
	EXT2_CHECK_MAGIC(channel, EXT2_ET_MAGIC_IO_CHANNEL);

	if (channel->manager->check_cache_blk)
		return (channel->manager->check_cache_blk)(channel, block);

	// if ((block >> 32) != 0)
	// 	return EXT2_ET_IO_CHANNEL_NO_SUPPORT_64;
	//
	// return (channel->manager->check_cache_blk)(channel, (unsigned long) block,
	// 				     count, data);
