...
static errcode_t alloc_icount(ext2_filsys fs, int flags, ext2_icount_t *ret)
{
	...
	*ret = 0;
	...
	// Force Fullmap for low complexity of locked icount
	if ( 1 || (flags & EXT2_ICOUNT_OPT_FULLMAP) &&
	    (flags & EXT2_ICOUNT_OPT_INCREMENT)) {
		unsigned sz = sizeof(*icount->fullmap) * icount->num_inodes;

		sz += sizeof(*icount->fullmap); // need one more slot for fullmap 
		                                // as last index is used. 
...
#ifdef _ICOUNT_MERGE
void ext2fs_icount_merge(ext2_icount_t icount_src, ext2_icount_t icount_dest){
	printf("Merging icount %p into icount %p\n", icount_src, icount_dest);
	...
	printf("Going to iterate over %d inos\n", icount_dest->num_inodes);
	for(ino = 0; ino <= icount_dest->num_inodes; ino++){
		ext2fs_icount_fetch(icount_src, ino, &src_val);

		if(src_val){
			//printf("Merging ino %d", ino);
			//printf("src: %d, dest: %d", src_val, dest_val);
			ext2fs_icount_fetch(icount_dest, ino, &dest_val);
			new_val = src_val + dest_val;
			//printf("newval: %d\n", new_val);
			ext2fs_icount_store(icount_dest, ino, new_val);
		}
