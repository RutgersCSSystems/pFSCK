...
#define CACHE_SIZE 8 + _READ_DIR_BLOCKS
#define WRITE_DIRECT_SIZE 4	/* Must be smaller than CACHE_SIZE */
#define READ_DIRECT_SIZE 4 + _READ_DIR_BLOCKS /* Should be smaller than CACHE_SIZE */
#else
#define CACHE_SIZE 8
#define WRITE_DIRECT_SIZE 4	/* Must be smaller than CACHE_SIZE */
...
static int check_cached_block(struct unix_private_data *data,
					    unsigned long long block, void *buf, int bytes)
{
	struct unix_cache	*cache; //, *unused_cache, *oldest_cache;
	int			i;
	...
	for (i=0, cache = data->cache; i < CACHE_SIZE; i++, cache++) {
		...
		if (cache->block == block) {
			cache->access_time = ++data->access_time;
			//printf("Copying block %llu (%d)from %p to buf %p\n", block, bytes, cache->buf, buf);
			//copy over
			if(buf){
				memcpy(buf, cache->buf, bytes);
			}
			return 1;
		}
		//if (!oldest_cache ||
			       ...
			       int count, void *buf)
{
...
#else	
	if (count < 0 || count > READ_DIRECT_SIZE){ 
		if ((retval = flush_cached_blocks(channel, data, 0))){
			return retval;
		}
		return raw_read_blk(channel, data, block, count, buf);
	...
	while (count > 0) {
		...
		if ((cache = find_cached_block(data, block, &reuse[0]))) {
			...
			//make sure to reuse cache so following calls pick another cache spot
			reuse_cache(channel, data, reuse[0], block);
		}
#endif
		if (count == 1) {
			/*
		...
		for (i=1; i < count; i++){
			if (find_cached_block(data, block+i, &reuse[i])){
				...
				// make sure to reuse cache so following calls pick another cache spot
				reuse_cache(channel, data, reuse[i], block+i);
			}
		}
#endif
#ifdef DEBUG
...
static int check_cached_blk(io_channel channel, unsigned long long block, void *buf)
{
	struct unix_private_data *data;
	//struct unix_cache *cache;
	//struct unix_cache *reuse[READ_DIRECT_SIZE];

#ifdef NO_IO_CACHE
	return 0; // return 0 if we're not using an io cache at all
#endif
	...
	//int i;
	//printf("Cache: ");
	//printf("gonna try and check cache for %llu  Cache: ", block);
	//for (i=0, cache = data->cache; i < CACHE_SIZE; i++, cache++) {
	//	printf(" %llu ", cache->block);
	//}
	//printf("\n");

	// if it returns cached block, just return 1
  	if(check_cached_block(data, block, buf, channel->block_size)){
		//cache_hits++;
...
static struct struct_io_manager struct_unix_manager = {
	...
	.check_cache_blk = check_cached_blk,
