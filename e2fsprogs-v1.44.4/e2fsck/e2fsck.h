...
struct dx_dir_info {
	...
	pthread_rwlock_t rwlock; // not used
...
// Forward declare Necessary Stucts and typedefs
struct thread_ctx;
typedef struct e2fsck_struct *e2fsck_t;
struct extent_tree_info;

// Problem.c copy of problem. cstructs
...
/* Generic Thread Context
 *
 * This context is used to pass any necessary variables needed
 * for a single thread to check inodes
 */
struct thread_ctx{
	//Thread information
	int tid;

	// Global Structures to Reuse
	ext2_filsys fs; // alias, main one will be in ctx->fs
	e2fsck_t ctx;
	...
	// isolated structures temp pointer
	struct dir_info_db *dir_info;
	ext2_dblist dblist;

	// Pass 1 Specific Context
	// maybe under pass1_local_vars struct? to do a simple memcpy?
	struct pass1_vars* pass1_ctx;
...
struct e2fsck_struct {
...
#endif
	struct vgroup *vgroups;

	struct pass1_vars *p1_vars_template;

	/* Variables for syncing checking with fixing*/
	// Used by pfsck_problem.c
	int active_count;
	pthread_mutex_t active_lock;
	int pending_fixes;
	pthread_mutex_t fix_lock;

	pthread_cond_t work_stalled;
	pthread_cond_t fixes_done;

	int next_fix_disabled;


	//
	// Need to have reachable thread counts for scheduler 

	// Schduler
	int threads; 
	int scheduled_threads;
	int active_data_threads;
	int active_pipeline_threads;
	pthread_mutex_t sched_lock;

	fsck_lock_t dx_dir_lock;
	// Thread pools
	threadpool tpool;
	...
	// Extra pass 1 statistics
	int total_groups_checked;
	int total_inodes_checked;
	int total_blocks_checked;
	int* group_counter;

	// Thread Contexts
	struct thread_ctx *tctx_array;	// list of thread contexts
	fsck_lock_t tctx_array_lock;	// lock for iterating over tctx array
	pthread_key_t tctx_key;		// thread key for getting threads specific tctx
	int tctx_count;			// total number of thread contexts
	pthread_key_t fs_key;		// Key for thread specifi fs struct

	// Aggregation elements located in vgroups
	// to maintain order
	// however counters are maintained per thread
...
extern int e2fsck_dir_info_get_dotdot(e2fsck_t ctx, ext2_ino_t ino,
				      ext2_ino_t *dotdot);
extern void setup_db_directly(e2fsck_t ctx, struct dir_info_db	**dir_info);
extern struct dir_info* e2fsck_create_dir_info_single(ext2_ino_t ino, ext2_ino_t parent);
extern void e2fsck_free_dir_info_single(struct dir_info* dir);
extern void e2fsck_add_dir_info_directly(e2fsck_t ctx, struct dir_info_db	*dir_info, ext2_ino_t ino, ext2_ino_t parent);
extern void e2fsck_free_dir_info_directly(struct dir_info_db	**db);
extern void e2fsck_merge_dir_info(e2fsck_t ctx, struct dir_info_db** dir_info1, struct dir_info_db** dir_info2);
...
#ifdef _PIPELINE_PARALLELISM
extern void setup_pass2_threads(e2fsck_t ctx, int pipeline_threads);
extern void breakdown_pass2_threads();
extern void e2fsck_pass2_task(void* args);
extern void process_db_list_so_far(e2fsck_t ctx, ext2_dblist dblist, struct dir_info_db* dir_info_ptr);
#endif
...
/* threads.c */
extern struct thread_ctx* get_tctx(e2fsck_t ctx);
extern void run_thread(void* args);
extern void schedule_data_thread(e2fsck_t ctx);
extern void unschedule_data_thread(e2fsck_t ctx);
extern void schedule_pipeline_thread(e2fsck_t ctx);
extern void unschedule_pipeline_thread(e2fsck_t ctx);
extern void turn_data_to_pipe(e2fsck_t ctx);
extern void turn_pipe_to_data(e2fsck_t ctx);
