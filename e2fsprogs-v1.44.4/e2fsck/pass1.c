...
static fsck_lock_t ea_inode_refs_lock;

// Locks
static fsck_lock_t icount_store_lock;

// Locks for mutual exclusion
static fsck_lock_t check_blocks_lock;

// Inode Bitmap Locks
static fsck_lock_t inode_used_map_lock; // ext2fs_inode_bitmap inode_used_map; 		/* Inodes which are in use */

static fsck_lock_t inode_bad_map_lock;  // ext2fs_inode_bitmap inode_bad_map; 		/* Inodes which are bad somehow */
static fsck_lock_t inode_dir_map_lock;  // ext2fs_inode_bitmap inode_dir_map;		/* Inodes which are directories */
static fsck_lock_t inode_bb_map_lock;  // ext2fs_inode_bitmap inode_bb_map; 		/* Inodes which are in bad blocks */
static fsck_lock_t inode_imagic_map_lock;// ext2fs_inode_bitmap inode_imagic_map; 	/* AFS inodes */
static fsck_lock_t inode_reg_map_lock;// ext2fs_inode_bitmap inode_reg_map; 		/* Inodes which are regular files*/

// Block Bitmap Locks
static fsck_lock_t block_found_map_lock; // ext2fs_block_bitmap block_found_map; 	/* Blocks which are in use */
static fsck_lock_t block_dup_map_lock; // ext2fs_block_bitmap block_dup_map; 		/* Blks referenced more than once */
static fsck_lock_t block_ea_map_lock;// ext2fs_block_bitmap block_ea_map; 		/* Blocks which are used by EA's */

// Generic Lock for counts
static fsck_lock_t file_count_lock;

static fsck_lock_t mark_block_used_lock;
static fsck_lock_t add_dir_info_lock;
...
// Forward decclar added check_blocks function
static void check_blocks2(e2fsck_t ctx, struct problem_context *pctx,
			 char *block_buf, const struct ea_quota *ea_ibody_quota, struct thread_ctx* tctx);
...
// Forward Declaration
static void process_inodes2(e2fsck_t ctx, char *block_buf, struct thread_ctx* tctx);
			      ...
			      struct ext2_ext_attr_entry *first, void *end)
{
	...
	for (entry = first;
		...
		ea_refcount_increment(ctx->ea_inode_refs, entry->e_value_inum,
				      0);
		fsck_unlock(&ea_inode_refs_lock);
	}
}
...
// Modified FINISH_INODE_LOOP MACRO for check_inodes_thread()
#define FINISH_INODE_LOOP_THREAD(ctx, ino, pctx, failed_csum, tctx) \
	do { \
		finish_processing_inode((ctx), (ino), (pctx), (failed_csum)); \
		if ((ctx)->flags & E2F_FLAG_ABORT) \
		  tctx->pass1_ctx->ret = -3; \
	} while (0)
...
static const char* get_next_inode_string;
#endif

#ifndef _INODE_THRESHOLD 
#define _INODE_THRESHOLD 25000
#endif

//#ifndef _DBLIST_THRESHOLD
//#define _DBLIST_THRESHOLD 1000
//#endif
...
struct thread_ctx* get_pass1_tctx(e2fsck_t ctx){

	// get tctx from thread specific 
	struct thread_ctx* tctx = get_tctx(ctx);

	// setup anything necessary for pass 1 if not already set up
	if( tctx->pass1_ctx == NULL ){

		tctx->pass1_ctx = (struct pass1_vars*) malloc(sizeof(struct pass1_vars));

		// Copy main local things
		memcpy(tctx->pass1_ctx, ctx->p1_vars_template, sizeof(struct pass1_vars));
		
		// initialize buffers
		tctx->pass1_ctx->block_buf = (char*) malloc(ctx->fs->blocksize * 3);
		tctx->pass1_ctx->inode = (struct ext2_inode *) malloc(tctx->pass1_ctx->bufsize);
		tctx->pass1_ctx->scan_struct.block_buf = tctx->pass1_ctx->block_buf;
	
		// setup scan
		ext2fs_open_inode_scan(tctx->fs, ctx->inode_buffer_blocks, &tctx->pass1_ctx->scan);
		
		ext2fs_inode_scan_flags(tctx->pass1_ctx->scan, EXT2_SF_SKIP_MISSING_ITABLE |
								EXT2_SF_WARN_GARBAGE_INODES, 0);
		ext2fs_set_inode_callback(tctx->pass1_ctx->scan, scan_callback, &tctx->pass1_ctx->scan_struct);

		// initalize counters
		
		tctx->pass1_ctx->fs_directory_count = 0;
		tctx->pass1_ctx->fs_regular_count = 0;
		tctx->pass1_ctx->fs_chardev_count = 0;
		tctx->pass1_ctx->fs_blockdev_count = 0;
		tctx->pass1_ctx->fs_symlinks_count = 0;
		tctx->pass1_ctx->fs_fast_symlinks_count = 0;
		tctx->pass1_ctx->fs_fifo_count = 0;
		tctx->pass1_ctx->fs_sockets_count = 0;
		tctx->pass1_ctx->fs_ind_count = 0;
		tctx->pass1_ctx->fs_dind_count = 0;
		tctx->pass1_ctx->fs_tind_count = 0;


#ifdef _ICOUNT_MERGE_A
		// setup isolated icount
		e2fsck_setup_icount(ctx, "inode_link_info", 0, NULL,
					   &ctx->tctx->inode_link_info);
#endif
	}
...
void start_vgroup(struct thread_ctx* tctx, struct vgroup *vgroup_t){

	// set fs for vgroup dblist
	ext2fs_dblist_set_fs(vgroup_t->dblist, tctx->fs);

	// start blockgroup 
	ext2fs_inode_scan_goto_blockgroup(tctx->pass1_ctx->scan, get_start_group(vgroup_t));
...
/* Check Inodes Thread
 *
 * Function used by a thread to check inodes given some context. Not to be
 * confused with process_inodes() function
 *
 * Simply a slightly modified version of the main while(1) loop that iterates over
 * over all the inodes in e2fsck_pass1().
 *
 * There are multiple break points,
 * the following return values describes the possible return values
 * stored in ret:
 *    1. Break = -1
 *    2. goto ENDIT = -2
 *    3. Return = -3
 *    4. *continue normally*  == -4 // This is default due to last inode check
 *
 * @arg args - void pointer to thread context object
 */
void check_inodes_thread(void *args)
{

	struct timeval start_time,stop; // Time structs for timing
	int last_inode = 0; // Flag to signify it its the last inode to be processed
	
	struct vgroup* vgroup_t = (struct vgroup*) args;

	printf("vgroup dblist is %p\n", vgroup_t->dblist);
	e2fsck_t ctx = (e2fsck_t) vgroup_t->ctx;

	// start syncing for fixes here:
	pfsck_sync_start(ctx);

	// Unpack thread context from void arguments
	struct thread_ctx* tctx = get_pass1_tctx(ctx); 

	// set tctx dblist to vgroup tctx to pass along various pass 1 calls
	tctx->dblist = vgroup_t->dblist;


	ext2_filsys fs = tctx->fs;



	tctx->pass1_ctx->start_inode = get_start_inode(vgroup_t);
	tctx->pass1_ctx->end_inode = get_end_inode(vgroup_t);
	tctx->pass1_ctx->start_group = get_start_group(vgroup_t);
	tctx->pass1_ctx->end_group = get_end_group(vgroup_t);
	ext2fs_inode_scan_goto_blockgroup(tctx->pass1_ctx->scan, get_start_group(vgroup_t));
	tctx->dir_info = (struct dir_info_db*) vgroup_t->dir_info;

	printf("Thread starting, checking from inode %d to %d\n", tctx->pass1_ctx->start_inode, tctx->pass1_ctx->end_inode);

	// Unpack local variables from main e2fsck_pass1() 
	ext2_ino_t ino = tctx->pass1_ctx->ino;
	const char *old_op = tctx->pass1_ctx->old_op;
	struct problem_context pctx = tctx->pass1_ctx->pctx;
	ext2_inode_scan scan = tctx->pass1_ctx->scan;
	struct ext2_inode *inode = tctx->pass1_ctx->inode;
	unsigned int inode_size = tctx->pass1_ctx->inode_size;
	int imagic_fs = tctx->pass1_ctx->imagic_fs;
	int extent_fs = tctx->pass1_ctx->extent_fs;
	int inlinedata_fs = tctx->pass1_ctx->inlinedata_fs;
	int low_dtime_check = tctx->pass1_ctx->low_dtime_check;
	int failed_csum = tctx->pass1_ctx->failed_csum;
	ext2_ino_t ino_threshold = tctx->pass1_ctx->ino_threshold;
	dgrp_t ra_group = tctx->pass1_ctx->ra_group;
	struct ext2_super_block *sb = tctx->pass1_ctx->sb;
	char *block_buf = tctx->pass1_ctx->block_buf;
	unsigned char frag = tctx->pass1_ctx->frag;
	unsigned char fsize = tctx->pass1_ctx->fsize;
	struct ea_quota ea_ibody_quota = tctx->pass1_ctx->ea_ibody_quota;
	...
	while (1) {
		...
		pfsck_sync_fix(ctx); //sync with any available fixes

		if(last_inode == 1){
			printf("ino: %d, end_inode: %d\n", ino, tctx->pass1_ctx->end_inode);
			//tctx->pass1_ctx->ret = -4;
			vgroup_t->ret = -1;
			break;
		}
...
#ifdef _REDUCE_OP_STRINGS
		old_op = ehandler_operation(get_next_inode_string);
#else
		...
		if (pctx.errcode == EXT2_ET_BAD_BLOCK_IN_INODE_TABLE) {
			...
			fsck_lock(&inode_bb_map_lock);
			if (!ctx->inode_bb_map)
				alloc_bb_map(ctx);
			ext2fs_mark_inode_bitmap2(ctx->inode_bb_map, ino);
			fsck_unlock(&inode_bb_map_lock);
			fsck_lock(&inode_used_map_lock);
			ext2fs_mark_inode_bitmap2(ctx->inode_used_map, ino);
			fsck_unlock(&inode_used_map_lock);
			continue;
		}
		if (pctx.errcode &&
		...
		if (!ino){
			vgroup_t->ret =  -1; //break;
			break;
		}

		// CHECK IF INODE IS IN BOUNDS
		if (ino < tctx->pass1_ctx->start_inode){
			//printf("ino: %d, start: %d", ino, tctx->start_inode);
			continue; // get next inode
		}

		// Ensure it will break out if this is the last inode
		// that should be processed
		if (ino == tctx->pass1_ctx->end_inode){
			last_inode = 1;
		}

		// At this point its going to check the inode
		if(ctx->extra_output){
			tctx->pass1_ctx->inodes_checked++;
			tctx->pass1_ctx->group_counter[get_group_no(scan)]++;
		}
		...
		if (inode->i_links_count) {
#ifdef _ICOUNT_MERGE_A
			pctx.errcode = ext2fs_icount_store(tctx->inode_link_info,
					ino, inode->i_links_count);
#else
			fsck_lock(&icount_store_lock);
			pctx.errcode = ext2fs_icount_store(ctx->inode_link_info,
						 ino, inode->i_links_count);
			fsck_unlock(&icount_store_lock);
#endif
			if (pctx.errcode) {
			...
			FINISH_INODE_LOOP_THREAD(ctx, ino, &pctx, failed_csum, tctx);
			if (vgroup_t->ret == -3)
				break;
			continue;
		}
		...
		// Catch original goto in switch cases
		if (vgroup_t->ret == -2)
			break;
		...
		if (ino == EXT2_BAD_INO) {
			...
			fsck_lock(&block_found_map_lock);
			pctx.errcode = ext2fs_copy_bitmap(ctx->block_found_map,
								&pb.fs_meta_blocks);
			fsck_unlock(&block_found_map_lock);
			...
			if (fs->super->s_journal_inum == EXT2_JOURNAL_INO) {
				...
				check_blocks2(ctx, &pctx, block_buf, NULL, tctx);
				FINISH_INODE_LOOP_THREAD(ctx, ino, &pctx, failed_csum, tctx);
				if (vgroup_t->ret == -3)
					break;
				continue;
...
#ifdef _ICOUNT_MERGE_A
				ext2fs_icount_store(tctx->inode_link_info,
					ino, 0);
#else
		...
		if (inode->i_flags & EXT2_IMAGIC_FL) {
			if (imagic_fs) {
				fsck_lock(&inode_imagic_map_lock);
				if (!ctx->inode_imagic_map)
					alloc_imagic_map(ctx);
				ext2fs_mark_inode_bitmap2(ctx->inode_imagic_map,
							 ino);
			  fsck_unlock(&inode_imagic_map_lock);
			} else {
		...
		if (LINUX_S_ISDIR(inode->i_mode)) {
			fsck_lock(&inode_dir_map_lock);
			ext2fs_mark_inode_bitmap2(ctx->inode_dir_map, ino);
			fsck_unlock(&inode_dir_map_lock);
			e2fsck_add_dir_info_directly(ctx, tctx->dir_info, ino, 0);
			tctx->pass1_ctx->fs_directory_count++;
			if (inode->i_flags & EXT4_ENCRYPT_FL)
				add_encrypted_dir(ctx, ino);
		} else if (LINUX_S_ISREG (inode->i_mode)) {
			fsck_lock(&inode_reg_map_lock);
			ext2fs_mark_inode_bitmap2(ctx->inode_reg_map, ino);
			tctx->pass1_ctx->fs_regular_count++;
			fsck_unlock(&inode_reg_map_lock);
		} else if (LINUX_S_ISCHR (inode->i_mode) &&
				 e2fsck_pass1_check_device_inode(fs, inode)) {
			check_extents_inlinedata(ctx, &pctx);
			check_immutable(ctx, &pctx);
			check_size(ctx, &pctx);
			tctx->pass1_ctx->fs_chardev_count++;
		} else if (LINUX_S_ISBLK (inode->i_mode) &&
				 e2fsck_pass1_check_device_inode(fs, inode)) {
			check_extents_inlinedata(ctx, &pctx);
			check_immutable(ctx, &pctx);
			check_size(ctx, &pctx);
			tctx->pass1_ctx->fs_blockdev_count++;
		} else if (LINUX_S_ISLNK (inode->i_mode) &&
				 e2fsck_pass1_check_symlink(fs, ino, inode,
									block_buf)) {
			check_immutable(ctx, &pctx);
			tctx->pass1_ctx->fs_symlinks_count++;
			if (inode->i_flags & EXT4_INLINE_DATA_FL) {
				FINISH_INODE_LOOP_THREAD(ctx, ino, &pctx, failed_csum, tctx);
				if (vgroup_t->ret == -3)
					break;
				...
				tctx->pass1_ctx->fs_fast_symlinks_count++;
				check_blocks2(ctx, &pctx, block_buf,
							 &ea_ibody_quota, tctx);
				FINISH_INODE_LOOP_THREAD(ctx, ino, &pctx, failed_csum, tctx);
				if (vgroup_t->ret == -3)
					break;
			...
			check_immutable(ctx, &pctx);
			check_size(ctx, &pctx);
			tctx->pass1_ctx->fs_fifo_count++;
		} else if ((LINUX_S_ISSOCK (inode->i_mode)) &&
				 e2fsck_pass1_check_device_inode(fs, inode)) {
			check_extents_inlinedata(ctx, &pctx);
			check_immutable(ctx, &pctx);
			check_size(ctx, &pctx);
			tctx->pass1_ctx->fs_sockets_count++;
		} else
			mark_inode_bad(ctx, ino);
		if (!(inode->i_flags & EXT4_EXTENTS_FL) &&
				!(inode->i_flags & EXT4_INLINE_DATA_FL)) {
			if (inode->i_block[EXT2_IND_BLOCK])
				tctx->pass1_ctx->fs_ind_count++;
			if (inode->i_block[EXT2_DIND_BLOCK])
				tctx->pass1_ctx->fs_dind_count++;
			if (inode->i_block[EXT2_TIND_BLOCK])
				tctx->pass1_ctx->fs_tind_count++;
		}
		if (!(inode->i_flags & EXT4_EXTENTS_FL) &&
				!(inode->i_flags & EXT4_INLINE_DATA_FL) &&
			...
			check_blocks2(ctx, &pctx, block_buf, &ea_ibody_quota, tctx);
		FINISH_INODE_LOOP_THREAD(ctx, ino, &pctx, failed_csum, tctx);
		if (vgroup_t->ret == -3)
			break;
		...
		if (process_inode_count >= ctx->process_inode_size) {
			process_inodes2(ctx, block_buf, tctx);
...
#ifndef _DBLIST_THRESHOLD 
		inode_ctr++;
		if(inode_ctr % _INODE_THRESHOLD == 0){
			process_db_list_so_far(ctx, tctx->dblist, tctx->dir_info);
		}
#else
		if(tctx->dblist->count - tctx->dblist->last_checked >= _DBLIST_THRESHOLD){
			process_db_list_so_far(ctx, tctx->dblist, tctx->dir_info);
		}
#endif
...
#ifdef _PIPELINE_PARALLELISM
	if(ctx->pipeline_threads || ctx->max_threads){
		process_db_list_so_far(ctx, tctx->dblist, tctx->dir_info);
	}
#endif

	// stop syncing with fixes here
	pfsck_sync_stop(ctx);
	...
	// Store time in context
 	tctx->pass1_ctx->time = ((double) (1000000 * (stop.tv_sec - start_time.tv_sec) + stop.tv_usec - start_time.tv_usec) / 1000000);

	// Print Out Statistics For Thread
	if(tctx->ctx->extra_output){
		printf("Thread %d processed %d inodes in %f seconds\n", tctx->tid, tctx->pass1_ctx->inodes_checked, ((double) (1000000 * (stop.tv_sec - start_time.tv_sec) + stop.tv_usec - start_time.tv_usec) / 1000000));
	}
...
/* Threaded Pass 1 Regular Functions*/
...
int print_pass_1_info(e2fsck_t ctx){
	ext2_filsys fs = ctx->fs;
	int total_inodes = fs->group_desc_count * EXT2_INODES_PER_GROUP(fs->super);
	...
	printf("\nRunning Threaded Inode Checking\n");

	// Print number of block groups
	printf("\nGroup Information\n------------------------\n");
	printf("# inodes per group * inode kb size / 4kb per block = # inode blocks\n");
	printf("Number of Groups: %d\n", fs->group_desc_count);
	printf("Number of inodes per group: %d\n", EXT2_INODES_PER_GROUP(fs->super));
	printf("Inode Blocks per group: %d\n", fs->inode_blocks_per_group);
	printf("Inode Bytes per group: %d\n", fs->inode_blocks_per_group * 4096);
...
int launch_threads(e2fsck_t ctx){
	printf("Launching threads pass1 scheduler threads\n");
	
	int x;	
	struct vgroup* vgroup_t; // virtual group pointer
	struct dir_info_db *dir_info;
	int split;

#ifdef _SCHEDULER
	if(ctx->scheduler){
		split = 64; // ctx->max_threads
#else
	if(0){
#endif	
	}else{
		split = ctx->data_threads;
	}
	ctx->work_split = split;
	// split work into max cores i.e. 16 then add to thread pool
	split_into_vgroups(ctx->fs, split, &ctx->vgroups);	
	
	for(x = 0; x < split; x++){
		get_vgroup(ctx->vgroups, x, &vgroup_t);
		vgroup_t->ctx = (void*) ctx;

		printf("Start inode %d, end inode: %d\n", vgroup_t->start_inode, vgroup_t->end_inode);

		setup_db_directly(ctx, &dir_info);
		vgroup_t->dir_info = (void*) dir_info;
		ext2fs_init_dblist(ctx->fs, &vgroup_t->dblist);

#ifdef _SCHEDULER
		if (ctx->scheduler){		
			thpool_add_work(ctx->tpool_1, (void*) check_inodes_thread, (void*) vgroup_t);
		}	
#endif
	}

	// start scheduler now once initial work has been loaded;
#ifdef _SCHEDULER
	if (ctx->scheduler){
		run_scheduler_thread(ctx->sched);		
#else
	if (0){
#endif
	}else{
		for(x = 0; x < split - 1; x++){
			get_vgroup(ctx->vgroups, x, &vgroup_t);
			thpool_add_work(ctx->tpool, (void*) check_inodes_thread, (void*) vgroup_t);		
		}
		get_vgroup(ctx->vgroups, x, &vgroup_t);
		check_inodes_thread((void*) vgroup_t);	
	}
	
	printf("Finished adding work to tpool_1\n");
	//exit(0);
	return 0;
}


/* Aggregates results from threads
 *
 */
int aggregate_results(e2fsck_t ctx){
	int x,y;

	struct thread_ctx* tctx_array = ctx->tctx_array;

	// aggregate dir info lists
	setup_db_directly(ctx, &ctx->dir_info);

	//if(ctx->extra_output){
	//	ctx->group_counter = (int*) malloc(sizeof(int) * ctx->fs->group_desc_count);
	//}
...
#ifdef _ICOUNT_MERGE_A
		if (tctx_array[x].inode_link_info != NULL){
			ext2fs_icount_merge(tctx_array[x].inode_link_info, ctx->inode_link_info);
			ext2fs_free_icount(tctx_array[x].inode_link_info);
		}
#endif
	
		//aggregate counters
		if (tctx_array[x].pass1_ctx){
			ctx->fs_directory_count += tctx_array[x].pass1_ctx->fs_directory_count;
			ctx->fs_regular_count += tctx_array[x].pass1_ctx->fs_regular_count;
			ctx->fs_chardev_count += tctx_array[x].pass1_ctx->fs_chardev_count;
			ctx->fs_blockdev_count += tctx_array[x].pass1_ctx->fs_blockdev_count;
			ctx->fs_symlinks_count += tctx_array[x].pass1_ctx->fs_symlinks_count;
			ctx->fs_fast_symlinks_count += tctx_array[x].pass1_ctx->fs_fast_symlinks_count;
			ctx->fs_fifo_count += tctx_array[x].pass1_ctx->fs_fifo_count;
			ctx->fs_sockets_count += tctx_array[x].pass1_ctx->fs_sockets_count;
			ctx->fs_ind_count += tctx_array[x].pass1_ctx->fs_ind_count;
			ctx->fs_dind_count += tctx_array[x].pass1_ctx->fs_dind_count;
			ctx->fs_tind_count += tctx_array[x].pass1_ctx->fs_tind_count;
		}
		// aggregate return value	
		//printf("Examining return: %d\n", tctx_array[x].pass1_ctx->ret);

	}

	for(x = 0; x < ctx->work_split; x++){
		
		// BY VGROUP

		// aggregate dblist
		ext2fs_dblist_merge(ctx->fs->dblist, ctx->vgroups[x].dblist);
		ext2fs_free_dblist_single(ctx->vgroups[x].dblist);


		e2fsck_merge_dir_info(ctx, &ctx->dir_info, (struct dir_info_db**) &ctx->vgroups[x].dir_info);
		e2fsck_free_dir_info_directly((struct dir_info_db**) &ctx->vgroups[x].dir_info);
	}



	for(x = 0; x < ctx->work_split; x++){
		switch(ctx->vgroups[x].ret){
			case -1:
				//printf("Breaking\n\n");
				return -1;
			case -2:
			...
			case -4:
				//printf("Continuing\n");
				break; // Do nothing, just continue // default
			default:
				printf("Aggregate Results should not get here");
				return -5;
		}
	}
...
int cleanup_pass_1_extras(e2fsck_t ctx){
	int x; // loop counter
	int count;
	struct thread_ctx* tctx_array = ctx->tctx_array;
...
#ifdef _SCHEDULER
	if(ctx->scheduler){
		count = ctx->max_threads;
	}else{
		count = ctx->data_threads;
	}
#else
	count = ctx->data_threads;
#endif

	// cleanup pass 1 things
	for(x = 0; x < ctx->tctx_count; x++){
		if(tctx_array[x].pass1_ctx){
			free(tctx_array[x].pass1_ctx->inode);
			free(tctx_array[x].pass1_ctx->block_buf);
			if(ctx->extra_output){
				free(tctx_array[x].pass1_ctx->group_counter);
			}
			ext2fs_close_inode_scan(tctx_array[x].pass1_ctx->scan);
			free(tctx_array[x].pass1_ctx);
		}
	}

	// clenaup group count
	free(ctx->group_counter);

	return 0;
}

int print_statistic(e2fsck_t ctx){
	// prints statistics from passs 1

	////
	/* Print Checking Statistic ----------------------- */
	// if(ctx->extra_output){
	// 	printf("\nOverall Statistics\n");
	// 	printf("  Total inodes checked: %d\n", total_inodes_checked);
	// 	printf("Group Summary:\n");
	// 	printf("Group #\tInodes Checked\n");
	// 	int num_populated_groups = 0;
	//
	//
	// 	for(x = 0; x <  fs->group_desc_count; x++){
	// 		if (inodes_checked_array[x] > 0){
	// 			num_populated_groups++;
	// 		}
	// 		printf("%d\t%d\n", x, inodes_checked_array[x]);
	// 	}
	// 	printf("Groups populated: %d/%d\n", num_populated_groups, fs->group_desc_count);
	// }
}
...
#undef EXT2_BPP
	...
	block_buf = (char *) e2fsck_allocate_memory(ctx, fs->blocksize * (3 + _READ_DIR_BLOCKS),
...
#ifdef _REDUCE_OP_STRINGS
		// translate ahead of time outside while loop
		get_next_inode_string = _("getting next inode from scan");
#endif

	int inode_ctr = 0;

	// Parallel Inode Checking ================================================ //
	if(ctx->pass1_opt && ctx->data_threads){
		int x, y; // Counters for loops

		// Print filesystem information
		print_pass_1_info(ctx);

		struct pass1_vars p1_vars;
		int aggregate_result;


		ctx->p1_vars_template = (struct pass1_vars*) malloc(sizeof(struct pass1_vars));


		// Save pass 1 local variable needed for inode checking
		ctx->p1_vars_template->ino = ino;
		ctx->p1_vars_template->old_op = old_op;
		ctx->p1_vars_template->pctx = pctx;
		ctx->p1_vars_template->inode_size = inode_size;
		ctx->p1_vars_template->imagic_fs = imagic_fs;
		ctx->p1_vars_template->extent_fs = extent_fs;
		ctx->p1_vars_template->inlinedata_fs = inlinedata_fs;
		ctx->p1_vars_template->low_dtime_check = low_dtime_check;
		ctx->p1_vars_template->failed_csum = failed_csum;
		ctx->p1_vars_template->ino_threshold = ino_threshold;
		ctx->p1_vars_template->ra_group = ra_group;
		ctx->p1_vars_template->sb = sb;
		ctx->p1_vars_template->frag = frag;
		ctx->p1_vars_template->fsize = fsize;
		ctx->p1_vars_template->ea_ibody_quota = ea_ibody_quota;
		ctx->p1_vars_template->scan_struct.ctx = ctx;
		ctx->p1_vars_template->bufsize = bufsize;
...
#ifdef _PIPELINE_PARALLELISM
		// can't aggregate dblists while pass 2 acting on it
		
#ifdef _SCHEDULER
		if(ctx->scheduler){
			thpool_wait(ctx->tpool_1);
			//borrow_threads(ctx->tpool_1, ctx->tpool_2, 4);
			thpool_wait(ctx->tpool_2);
			kill_scheduler_thread(ctx->sched);
		}else if(ctx->extra_threads){
		...
		aggregate_result = aggregate_results(ctx);

		// Cleanup
		cleanup_pass_1_extras(ctx);

		// Act on aggregated result
		switch(aggregate_result){
			case -1:
				goto end_while; // this is the corect path
			case -2:
				goto endit;
			case -3:
				return;
			default:
				printf("[Act on p1 aggregated result] - SHOULD NOT GET HERE (%d)\n", aggregate_result);
			 	return;
		}
	...
	while (1) {
		...
		if (LINUX_S_ISDIR(inode->i_mode)) {
			...
			// Single dir_info struct
			//struct dir_info* dir_to_process = e2fsck_create_dir_info_single(ino, 0);
			//e2fsck_free_dir_info_single(dir_to_process);

			// Actual use an actual list
			//struct dir_info_db *dir_info_list;
			//setup_db_directly(ctx, &dir_info_list);
			//e2fsck_add_dir_info_directly(ctx, dir_info_list, ino, 0);
			//e2fsck_free_dir_info_directly(&dir_info_list);

			//e2fsck_pass2_task(NULL);
...
#ifndef _DBLIST_THRESHOLD
		inode_ctr++;
		if(ctx->pipeline_threads && inode_ctr % _INODE_THRESHOLD){
			process_db_list_so_far(ctx, ctx->fs->dblist, ctx->dir_info);
		}
#else
		if(ctx->pipeline_threads &&
		   ctx->fs->dblist->count - ctx->fs->dblist->last_checked >= _DBLIST_THRESHOLD){
			process_db_list_so_far(ctx, ctx->fs->dblist, ctx->dir_info);
		}
#endif
#endif
...
endit:
	...
	ctx->pass1_time = ((double) (1000000 * (time_stop.tv_sec - time_start.tv_sec)\
...
static void process_inodes(e2fsck_t ctx, char *block_buf){
	process_inodes2(ctx, block_buf, NULL);
}
...
static void mark_inode_bad(e2fsck_t ctx, ino_t ino)
{
	...
	fsck_unlock(&inode_bad_map_lock);
}
...
static _INLINE_ void mark_block_used(e2fsck_t ctx, blk64_t block)
{
	...
	if (ext2fs_fast_test_block_bitmap2(ctx->block_found_map, block)) {
		//printf("Dup block found %u\n", block);
		fsck_unlock(&block_found_map_lock);
		if (ext2fs_has_feature_shared_blocks(ctx->fs->super) &&
		    !(ctx->options & E2F_OPT_UNSHARE_BLOCKS)) {
			return;
		}
		fsck_lock(&block_dup_map_lock);
		if (!ctx->block_dup_map) {
		//	printf("Going to ALlocate Dup block map\n");
			...
			if (pctx.errcode) {
				...
				/* Should never get here */
				ctx->flags |= E2F_FLAG_ABORT;
				fsck_unlock(&block_dup_map_lock);
				return;
			}
		}
 ...
 */
 fsck_lock_t mark_blocks_used_lock;
				      ...
				      unsigned int num)
{
	...
	if (ext2fs_test_block_bitmap_range2(ctx->block_found_map, block, num)){
		...
		for (i = 0; i < num; i += EXT2FS_CLUSTER_RATIO(ctx->fs)){
			fsck_lock(&mark_blocks_used_lock);
			mark_block_used(ctx, block + i);
			fsck_unlock(&mark_blocks_used_lock);
		}
	}
}
...
fsck_lock_t handle_htree_lock;
static int handle_htree(e2fsck_t ctx, struct problem_context *pctx,
			ext2_ino_t ino, struct ext2_inode *inode,
			char *block_buf)
{
	fsck_lock(&handle_htree_lock);
	...
	// If there is some parallelism going on
	// make sure to individual data thread fs structs
	int tid;
	struct thread_ctx *tctx;
	if(ctx->data_threads){
		tctx = get_tctx(ctx);
...
fsck_lock_t check_blocks_extents_lock;


// forward declare scan extent node for recursive calls
...
// old scan_extent_node
static void scan_extent_node2(e2fsck_t ctx, struct problem_context *pctx,
			     struct process_block_struct *pb,
			     blk64_t start_block, blk64_t end_block,
			     blk64_t eof_block,
			     ext2_extent_handle_t ehandle,
...
report_problem:
		...
		if (!is_leaf) {
			...
			if(tctx){
				scan_extent_node2(ctx, pctx, pb, extent.e_lblk,
						 last_lblk, eof_block, ehandle,
						 next_try_repairs, tctx);
			} else{
...
alloc_later:
	...
	next:
		fsck_lock(&check_blocks_extents_lock);
		pctx->errcode = ext2fs_extent_get(ehandle,
						  EXT2_EXTENT_NEXT_SIB,
						  &extent);
		fsck_unlock(&check_blocks_extents_lock);
	}
...
static void scan_extent_node(e2fsck_t ctx, struct problem_context *pctx,
			     ...
			     int try_repairs)
{
	scan_extent_node2(ctx, pctx, pb, start_block, end_block,
				     eof_block, ehandle, try_repairs, NULL);

}

// old check blocks extents
static void check_blocks_extents2(e2fsck_t ctx, struct problem_context *pctx,
				 struct process_block_struct *pb, struct thread_ctx* tctx)
{
	fsck_lock(&check_blocks_extents_lock);
	struct ext2_extent_info info;
	struct ext2_inode	*inode = pctx->inode;
	...
	fsck_lock(&check_blocks_extents_lock);
	if (tctx){
		scan_extent_node2(ctx, pctx, pb, 0, 0, eof_lblk, ehandle, 1, tctx);
	} else{
...
static void check_blocks_extents(e2fsck_t ctx, struct problem_context *pctx,
				 struct process_block_struct *pb)
{
	check_blocks_extents2(ctx, pctx, pb, NULL);
}


/* (old check_blocks_inline_data)
 ...
 */
static void check_blocks_inline_data2(e2fsck_t ctx, struct problem_context *pctx,
				     struct process_block_struct *pb, struct thread_ctx* tctx)
{
	int	flags;
	...
	if(tctx){
		pctx->errcode = ext2fs_add_dir_block2(tctx->dblist, pb->ino, 0, 0);
	} else{
	...
	if(tctx){
		pctx->errcode = ext2fs_add_dir_block2(tctx->dblist, pb->ino, 0, 1);
	} else{
...
static void check_blocks_inline_data(e2fsck_t ctx, struct problem_context *pctx,
				     struct process_block_struct *pb)
{
	check_blocks_inline_data2(ctx, pctx, pb, NULL);
}
			 ...
			 char *block_buf, const struct ea_quota *ea_ibody_quota, struct thread_ctx* tctx)
{
	...
	if (inlinedata_fs && (inode->i_flags & EXT4_INLINE_DATA_FL)){
		...
		if(tctx){
			check_blocks_inline_data2(ctx, pctx, &pb, tctx);
		} else{
		...
		if (extent_fs && (inode->i_flags & EXT4_EXTENTS_FL)){
			...
			if(tctx){
					check_blocks_extents2(ctx, pctx, &pb, tctx);
			} else{
...
// new check_blocks hook, passes in null
static void check_blocks(e2fsck_t ctx, struct problem_context *pctx,
			 char *block_buf, const struct ea_quota *ea_ibody_quota)
{
	check_blocks2(ctx, pctx, block_buf, ea_ibody_quota, NULL);
}
		  ...
		  void *priv_data)
{
	...
	if (p->is_dir && blockcnt == 0 && blk == 0) {
		...
		if(p->tctx){
			pctx->errcode = ext2fs_add_dir_block2(p->tctx->dblist, p->ino, 0, 0);
		} else{
...
mark_dir:
	if (p->is_dir && (blockcnt >= 0)) {
		while (++p->last_db_block < blockcnt) {
			...
			if(p->tctx){
				pctx->errcode = ext2fs_add_dir_block2(p->tctx->dblist,
											p->ino, 0,
											p->last_db_block);
			} else{
		...
		if(p->tctx){
			pctx->errcode = ext2fs_add_dir_block2(p->tctx->dblist, p->ino,
										blk, blockcnt);
		} else {
