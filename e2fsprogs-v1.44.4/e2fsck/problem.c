...
int fix_problem(e2fsck_t ctx, problem_t code, struct problem_context *pctx)
{
	...
	pfsck_pre_fix(ctx); // call pre_fix routine

	
	//printf("Problems so far: %d\n", ++num_probs);
	/*
	if(num_probs == 1000){
		ctx->single_thread = 1;
	}
	if(num_probs > 1000){
		printf("Continuing single threaded");
	}
	*/
	...
	if (!ptr) {
		...
		pfsck_post_fix(ctx); // call post_fix routine before returning
	...
	pfsck_post_fix(ctx); // run post fix routine before returning
