...
static errcode_t PRS(int argc, char *argv[], e2fsck_t *ret_ctx)
{
	...
	*ret_ctx = ctx;
	...
	while ((c = getopt(argc, argv, "panyrcC:B:dE:fvtFVM:b:I:j:P:l:L:N:SsDkz:xq:w:og:Z:X")) != EOF)
		switch (c) {
		case 'C':
			ctx->progress = e2fsck_update_progress;
	...
	_("Only one of the options -p/-a, -n or -y may be specified."));
		...
		case 'o':
			...
			break;
		case 'q':
			ctx->data_threads = atoi(optarg);
			// neeed one less than needed as main thread will be utilized
			ctx->extra_threads += ctx->data_threads - 1;
			printf("Arg Parsed: %d threads for data parallelism\n", ctx->data_threads);
			break;
#ifdef _PIPELINE_PARALLELISM
		case 'w':
			ctx->pipeline_threads = atoi(optarg);
			ctx->extra_threads += ctx->pipeline_threads;
			printf("Arg Parsed: %d threads for pipeline parallelism\n", ctx->pipeline_threads);
			break;
#endif
		case 'g':
			for(arg_counter = 0; arg_counter < strlen(optarg); arg_counter++){
				switch (optarg[arg_counter]) {
					case '1':
						ctx->pass1_opt = 1;
						ctx->optimize = 1;
						break;
					...
					case '5':
						...
						break;
					default:
						printf("(-g) %c is not a valid optimization.\n", optarg[arg_counter]);
						break;
				}
			}
			break;
...
#ifdef _SCHEDULER
			if(ctx->optimize){
				printf("ARG PARSED: Scheduler\n");
				ctx->scheduler = 1;
				ctx->max_threads = atoi(optarg);
			}
#else
			printf("ERROR: SCHEDULER NOT COMPILED to use, Compile to use -Z flag\n");
#endif
			break;
		case 'X':
#ifdef _SCHEDULER	
			printf("Arg Parsed: Using IDLE Scheduler Policy\n");
			ctx->sched_p.sched_priority = 0;
			sched_setscheduler(getpid(), SCHED_IDLE, &ctx->sched_p);	
			if(sched_getscheduler(getpid() == SCHED_IDLE)){
				printf("Scheduler Policy is now set to SCHED_IDLE\n");
			}else{
				printf("Scheduler Policy FAILED to be set to SCHED_IDLE\n");
			}

			//ioprio_set(IOPRIO_WHO_PROCESS, 0, IOPRIO_CLASS_IDLE);

			//if(ioprio_get(IOPRIO_WHO_PROCESS, 0) == IOPRIO_CLASS_IDLE){
			//	printf("Set io priority to idle\n");
			//}else{
			//	printf("Could not get io priotity to idle\n");
			//}
#else
			printf("ERRO: SCHEDULER NOT COMPILED TO USE");
#endif
			break;
		default:
			usage(ctx);
...
restart:
	...
	if ((ctx->options & E2F_OPT_READONLY) == 0) {
		...
		if (!(ctx->mount_flags & EXT2_MF_ISROOT &&
			...
			// Only make open exclusive access if there are no extra threads
			// trying to open and read and/or write the same filesystem
	...
	if(ctx->extra_threads){
		...
		// If not open as read only (as in the case with -y flag)
		// temporarily set flags to open reopen extra fs as readonly

		/*
		if ((ctx->options & E2F_OPT_READONLY) == 0){
			flags &= ~EXT2_FLAG_RW;

			if ((flags & EXT2_FLAG_EXCLUSIVE) != 0){
				exc_set = 1;
				flags &= ~EXT2_FLAG_EXCLUSIVE;
			}
		}
		*/
		...
		ctx->fss = (ext2_filsys *) malloc(sizeof(ext2_filsys) * 80);
		for(x = 0; x < 80; x++){
			// setup io ptr
			io_ptr = unix_io_manager;
			//e2fsck_setup_tdb(ctx, &io_ptr);
			try_open_fs(ctx, flags, io_ptr, &ctx->fss[x]);

			// Point to things not allocated since coundn't reread
			//if(ctx->fss[x]->super == NULL){
			//	ctx->fss[x]->super = fs->super;
			//}
			//if(ctx->fss[x]->group_desc == NULL){
			//	ctx->fss[x]->group_desc = fs->group_desc;
			//}
		}
...
skip_write:
	...
	if(ctx->extra_threads){
		//int x;
		for(x = 0; x < 80; x++){
			// XXXXXX ext2fs_close_free(&ctx->fss[x]);
			// Free filesystem structs manually
			// instead of closing them to prevent
			// extra csum writes to superblock
			ext2fs_free(ctx->fss[x]->device_name);	
			ext2fs_free(ctx->fss[x]->super);
			ext2fs_free(ctx->fss[x]->orig_super);	
			ext2fs_free(ctx->fss[x]->group_desc);
			ext2fs_free(ctx->fss[x]);
		}
		free(ctx->fss);
	}
