...
void check_parent(e2fsck_t ctx, struct dir_info *dir, struct problem_context *pctx);
...
#ifdef THR_DEBUG
fsck_lock_t p_lock = PTHREAD_MUTEX_INITIALIZER;
unsigned long long cc_it = 0;
unsigned long long pc_it = 0;
#endif

fsck_lock_t problem_lock = PTHREAD_MUTEX_INITIALIZER;
fsck_lock_t inode_done_map_lock = PTHREAD_MUTEX_INITIALIZER;

static int check_connectivity(e2fsck_t ctx, ext2_ino_t dir,
		struct problem_context *pctx, struct dir_info *last_lookup,
		ext2fs_inode_bitmap *thr_loop_bitmap);

struct dir_iter_args {
	int id;
	int dir_count;
	e2fsck_t ctx;
	int base;
...
static int end_early = 0;

/*
 * Function for thread that will be iterating through
 * directories and checking they are connected.
 */
void dir_iter_thread_entry(void* arg) {
#ifdef THR_DEBUG
	struct timespec thr_start, thr_end;
	clock_gettime(CLOCK_MONOTONIC, &thr_start);
#endif

	struct dir_iter_args* args = (struct dir_iter_args*)arg;
	struct dir_info* dir;
	struct problem_context	pctx;
	clear_problem_context(&pctx);
	struct dir_info_iter* iter = e2fsck_dir_info_iter_begin(args->ctx);
	ext2fs_inode_bitmap thr_inode_loop_detect = 0;
	int i = 0;

	/*
	 * This is normally part of e2fsck_dir_info_get_parent()
	 * and e2fsck_dir_info_get_dotdot(), but it has been changed
	 * to a thread scope variable instead.
	 */
	struct dir_info *last_lookup;

	/*
	 * set bounds for thread's iteration
	 */
	int end = args->base + args->work_size;

#ifdef THR_DEBUG
	printf("Thread start check connectivity %d\n", args->id);
	printf("work_size: %d base: %d end: %d\n", args->work_size, args->base, end);
#endif
	clock_t cthr_start = clock();
	for (i = args->base; i < end; ++i) {
#ifdef THR_DEBUG
	__sync_fetch_and_add(&cc_it, 1);
#endif
		if (args->ctx->flags & E2F_FLAG_SIGNAL_MASK ||
				args->ctx->flags & E2F_FLAG_RESTART) {
			end_early = 1;
			e2fsck_dir_info_iter_end(args->ctx, iter);
			return;
		}

		/*
		 * Gets a directory, tests that it is a directory,
		 * and then runs connectivity checking on directory
		 */
		if (i >= args->dir_count)
			break;

		dir = get_dir_at(args->ctx->dir_info, i);

		if (!dir)
			break;
		last_lookup = dir;
		if (ext2fs_test_inode_bitmap2(args->ctx->inode_dir_map, dir->ino)) {
			if (check_connectivity(args->ctx, dir->ino, &pctx, last_lookup, &thr_inode_loop_detect))
				break;
		}
		if (end_early)
			break;
...
#ifdef THR_DEBUG
	clock_gettime(CLOCK_MONOTONIC, &thr_end);
	printf("thread time: %llu usec \n", (((unsigned long long)thr_end.tv_sec - thr_start.tv_sec)*1000000+((unsigned long long)thr_end.tv_nsec - thr_start.tv_nsec))/1000);
	printf("thread time: %llu clocks \n", (unsigned long long)clock() - cthr_start);
#endif
}

/*
 * Multithreaded connectivity check
 * For each directory, marks it done and then moves up the
 * tree until it finds another directory marked done.
 */
static int check_connectivity(e2fsck_t ctx, ext2_ino_t dir,
		struct problem_context *pctx, struct dir_info *last_lookup,
		ext2fs_inode_bitmap *thr_loop_bitmap)
{
	...
	int		loop_pass = 0, parent_count = 0, loop_check = 0;
	ext2fs_inode_bitmap thr_inode_loop_detect = *thr_loop_bitmap;
	...
	while (1) {
		 ...
		 * return from this function, the inode will either be
		 ...
		 */
		fsck_lock(&inode_done_map_lock);
		if (ext2fs_mark_inode_bitmap2(inode_done_map, ino)) {
			fsck_unlock(&inode_done_map_lock);
			break;
		}
		fsck_unlock(&inode_done_map_lock);

		if (last_lookup && last_lookup->ino == ino) {
			parent = last_lookup->parent;
		} else {
			if (e2fsck_dir_info_get_parent(ctx, ino, &parent)) {
				fsck_lock(&problem_lock);
				fix_problem(ctx, PR_3_NO_DIRINFO, pctx);
				fsck_unlock(&problem_lock);
				return 0;
			}
		}
		...
		loop_check = 0;
		if (loop_pass)
			loop_check = ext2fs_test_inode_bitmap2(thr_inode_loop_detect, parent);

		if (!parent || loop_check) {
			fsck_lock(&problem_lock);
			if (e2fsck_dir_info_get_parent(ctx, ino, &parent)) {
				fix_problem(ctx, PR_3_NO_DIRINFO, pctx);
				fsck_unlock(&problem_lock);
		...
		if (loop_pass) {
			ext2fs_mark_inode_bitmap2(thr_inode_loop_detect, ino);
			 ...
			 */
			loop_pass = 1;
			if (thr_inode_loop_detect) {
				ext2fs_clear_inode_bitmap(thr_inode_loop_detect);
			} else {
				fsck_lock(&problem_lock);
				pctx->errcode = e2fsck_allocate_inode_bitmap(fs, _("inode loop detection bitmap"), EXT2FS_BMAP64_AUTODIR, "thr_inode_loop_detect", &inode_loop_detect);
				if (pctx->errcode) {
					pctx->num = 1;
					fix_problem(ctx,
...
/*
 * Function for thread that checks dotdot and parent
 * directory of each directory.
 */
void dir_parent_check_thread_entry(void* arg) {
	...
	int end = args->base + args->work_size;
#ifdef THR_DEBUG
	printf("Thread start check parents %d\n", args->id);
	printf("work_size: %d base: %d end: %d\n", args->work_size, args->base, end);
#endif

	for (i = args->base; i < end; ++i) {
#ifdef THR_DEBUG
	__sync_fetch_and_add(&pc_it, 1);
#endif
		if (i >= args->dir_count)
			break;
		dir = get_dir_at(args->ctx->dir_info, i);
		...
		if (!dir)
			break;
		check_parent(args->ctx, dir, &pctx);
	}
}
...
void check_parent(e2fsck_t ctx, struct dir_info *dir, struct problem_context *pctx) {
	...
	if (pctx->ino2 != pctx->dir) {
		...
		if (fix_problem(ctx, PR_3_BAD_DOT_DOT, pctx)) {
			fix_dotdot(ctx, dir->ino, pctx->dir);
		}
		fsck_unlock(&problem_lock);
	}
...
void e2fsck_pass3(e2fsck_t ctx)
{
	...
	int thread_count = ctx->data_threads;
...
#endif
#ifdef EXP_TEST
	struct timespec start_time, final_time;
	clock_gettime(CLOCK_MONOTONIC, &start_time);
#endif
	if (ctx->pass3_opt && thread_count) {
		/* Thread setup */
		end_early = 0;
		int dargs_size = thread_count * sizeof(struct dir_iter_args);
		struct dir_iter_args* dargs = malloc(dargs_size);
		memset(dargs, 0, sizeof(dargs_size));
		int dir_count = get_dir_count(ctx->dir_info);
		struct dir_iter_args darg;
		int thr_work = dir_count / thread_count + 1;

		int i = 0;
		/* Connectivity check */
		for (i = 0; i < thread_count; ++i) {
			memset(&darg, 0, sizeof(struct dir_iter_args));
			darg.ctx = ctx;
			darg.dir_count = dir_count;
			darg.id = i;
			darg.base = i * thr_work;
			darg.work_size = thr_work;
			dargs[i] = darg;
			if (i)
				thpool_add_work(ctx->tpool, dir_iter_thread_entry, (void*)&dargs[i]);
		}
		dir_iter_thread_entry((void *)&dargs[0]);
		if (thread_count > 1)
			thpool_wait(ctx->tpool);

		int final_dir_count = get_dir_count(ctx->dir_info);
		int dir_diff =  final_dir_count - dir_count;

		/* If there are more directories than before check them */
		if (dir_diff > 0) {
			memset(&darg, 0, sizeof(struct dir_iter_args));
			darg.base = dir_count - 1;
			darg.work_size = dir_diff;
			darg.ctx = ctx;
			darg.dir_count = final_dir_count;
			dir_iter_thread_entry((void *)&darg);
		}

		/* Parent/dotdot check */
		if (!end_early) {
			int thr_work = final_dir_count / thread_count + 1;
			int i = 0;
			for (i = 0; i < thread_count; ++i) {
				darg.base = i * thr_work;
				darg.dir_count = final_dir_count;
				darg.work_size = thr_work;
				if (i)
					thpool_add_work(ctx->tpool, dir_parent_check_thread_entry, (void*)&dargs[i]);
			}
			dir_parent_check_thread_entry((void *)&dargs[0]);
			if (thread_count > 1)
				thpool_wait(ctx->tpool);
		}
...
#ifdef EXP_TEST
#ifdef THR_DEBUG
	printf("clock_gettime: %llu usec \n", ((unsigned long long)final_time.tv_sec - start_time.tv_sec) * 1000000 + (final_time.tv_nsec - start_time.tv_nsec)/1000);
	printf("clock %ld usec\n", ((1000000*end_clock)/CLOCKS_PER_SEC));
#endif
	printf("dircpass3\t%llu\n", ((unsigned long long)final_time.tv_sec - start_time.tv_sec) * 1000000 + (final_time.tv_nsec - start_time.tv_nsec)/1000);
#endif
